...
/**
 * DB 연결을 시도하는 DB.init() 함수는 async 키워드가 부착되어 있다
 * 이 함수는 동기방식으로 실행되는데
...
// 이 함수의 return 값을받기 위해서는 ,then() 함수를 통하여 받아야 한다
...
router.get("/", (req, res) => {
  ...
  dbConn
    ...
    // akrdir 실행중 오류가 발생하면 .catch() 함수에 결과를 전달한다.
    .catch((err) => {
...
router.get("/:isbn/detail", (req, res) => {
  ...
  //     return res.json(err);
  //   } else {
  //     return res.render("books/detail", { books: result[0] });
  //   }
  // });
});
