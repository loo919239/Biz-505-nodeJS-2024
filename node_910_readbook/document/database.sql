...
정수값 : TINYINT, SMALINT, INT(INTEGER), BIGINT
    ...
    BIGING : 0 ~ 2의 8승 - 1 (- 2의 7승 ~ 2의 7승 -1)
...
-- DB 명령어를 실행하기 전에 MySQL 에서는 반드시 SCHEMA 를 연결해야한다
USE bookdb2;
...
일부 다른 DB 소프츠웨어는 임시기억장소에 아예 데이터를 저장해 두고, 어떤 명령등이 실행되면
...
/*
DDL(Data Definition Lang.) :DBA(Database Adminstrator, 최고관리자) 가 사용하는 명령어
...
-- DB의 물리적인 저장소를 생성, 제거, 변경 할 수 있는 권한을 갖는다
-- 일반적으로 현업에서는 root 사용자 외에 별도의 사용자를 생성하는
-- 제한적인 권한을 부여하여 사용하도록 한다.

-- loo919239 라는 사용자를 localhost 에서만 접근할수 있도록 생성한다
CREATE USER 'loo919239'@'localhost' IDENTIFIED BY '!Biz8080';
...
-- loo919239 사용자에게 bookDB2 데이터베이스에 대한 권한을 부여하기
-- TABLE 생성, CRUD를 수행할 수 있다
GRANT ALL PRIVILEGES ON bookDB2.* TO 'loo919239'@'localhost';
GRANT ALL PRIVILEGES ON *.* TO 'loo919239'@'localhost';
FLUSH PRIVILEGES;
-- 기존에 부여된 권한 회수하기
