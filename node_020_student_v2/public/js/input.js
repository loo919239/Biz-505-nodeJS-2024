document.addEventListener("DOMContentLoaded", () => {
  ...
  const error_div = document.querySelectorAll("div.student.error");
  /**
   * 먼저 btn_submit(button) 의 click event 를 최소한으로 테스트를 하고
   * 이후에 btn_submit null plinter exception 을 일으키는 현상을 방지하기 위하여
   * btn_subnit? 형식으로 이후 코드를 사용한다
   */
  btn_submit?.addEventListener("click", () => {
    // 표시되었던 error 메세지를 모두 clear
    error_div.forEach((item) => (item.innerHTML = ""));

    // alert("전송");
    if (!st_num.value) {
      error_div[ST_INDEX.ST_NUM].innerHTML = " * 학번은 반드시 입력하세요";
      st_num.select();
      return false;
    }
    if (!st_name.value) {
      error_div[ST_INDEX.ST_NAME].innerHTML = " * 학생의 이름은 반드시 입력해야 합니다";
      st_name.select();
      return false;
    }
    if (!st_dept.value) {
      error_div[ST_INDEX.ST_DEPT].innerHTML = " * 학과는 반드시 입력해야 합니다";
      st_dept.select();
      return false;
    }
